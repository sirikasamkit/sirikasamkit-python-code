...
def get_range_hint(number, current_min=1, current_max=100):
    ...
    return f"HINT:the nattowed range arount the number is {range(number - 12,number + 12)}"

def get_thefirst_digit_hint(number):
    # Retun the first digit of the number
    first_digit = number // 10
    return f"HINT:the first digiit is {first_digit}"
...
while True:
    ...
    if attempt == 3:
        ...
        print("get_thefirst_hint(random_number)")
